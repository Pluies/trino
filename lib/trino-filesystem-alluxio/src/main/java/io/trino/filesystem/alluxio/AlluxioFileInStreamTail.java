...
/**
 * Copy paste of io.trino.hdfs.FSDataInputStreamTail
 */
public final class AlluxioFileInStreamTail
{
    ...
    private AlluxioFileInStreamTail(long fileSize, Slice tailSlice)
    {
    ...
    public static AlluxioFileInStreamTail readTail(String path, long paddedFileSize, FileInStream inputStream, int length)
            throws IOException
    {
        ...
        return new AlluxioFileInStreamTail(position + bytesRead, Slices.wrappedBuffer(buffer, max(0, bytesRead - length), min(bytesRead, length)));
    }
