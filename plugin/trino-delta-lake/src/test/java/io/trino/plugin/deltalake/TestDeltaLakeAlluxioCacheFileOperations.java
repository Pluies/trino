...
import io.trino.plugin.deltalake.cache.DeltaLakeAlluxioFileSystemCache;
        ...
        extends AbstractTestQueryFramework
{
            ...
            throws Exception
    {
        ...
        try {
            ...
            alluxioFileSystemCache = new TestingAlluxioFileSystemCache(AlluxioFileSystemCacheModule.getAlluxioConfiguration(alluxioFileSystemCacheConfiguration));
            TrinoFileSystemFactory fileSystemFactory = new CacheFileSystemFactory(trackingFileSystemFactory, new DeltaLakeAlluxioFileSystemCache(alluxioFileSystemCache));

            queryRunner.installPlugin(new TestingDeltaLakePlugin(Optional.empty(), Optional.of(fileSystemFactory), binder -> binder.bind(NodeProvider.class).to(NoneNodeProvider.class).in(SINGLETON)));
    ...
    public void testCheckpointFileOperations()
    {
        ...
        assertFileSystemAccesses(
                ...
                ImmutableMultiset.<FileOperation>builder()
                        ...
                        .addCopies(new FileOperation(CHECKPOINT, "00000000000000000002.checkpoint.parquet", INPUT_FILE_NEW_STREAM), 3)
                        .addCopies(new FileOperation(CHECKPOINT, "00000000000000000002.checkpoint.parquet", INPUT_FILE_GET_LENGTH), 6)
        ...
        assertFileSystemAccesses(
                ...
                ImmutableMultiset.<FileOperation>builder()
                        ...
                        .addCopies(new FileOperation(CHECKPOINT, "00000000000000000004.checkpoint.parquet", INPUT_FILE_NEW_STREAM), 3)
                        .addCopies(new FileOperation(CHECKPOINT, "00000000000000000004.checkpoint.parquet", INPUT_FILE_GET_LENGTH), 6)
                        .addCopies(new FileOperation(DATA, "key=p1/", INPUT_FILE_NEW_STREAM), 1)
                        .addCopies(new FileOperation(DATA, "key=p2/", INPUT_FILE_NEW_STREAM), 1)
    ...
    private Multiset<CacheOperation> getCacheOperations()
    {
        return alluxioFileSystemCache.getOperationCounts()
                .entrySet().stream()
                .flatMap(entry -> nCopies(entry.getValue(), CacheOperation.create(
                        entry.getKey().type(),
